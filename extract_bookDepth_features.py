...
def extract_features(input_dir: str, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    # 1) Tages-Index von real_start bis user_ed
    dates = pd.date_range(
    ...
    # Falls inception > end_date, leeres DF zurückgeben
    if dates.empty:
        logging.warning(
            "⚠️ %s: Inception (%s) nach End-Datum (%s) – schreibe leeres DataFrame",
            symbol, inception.date(), user_ed.date()
        )
        df_empty = pd.DataFrame([], index=pd.DatetimeIndex([], tz="UTC"))
    ...
    for day in dates:
        ...
        if os.path.exists(fp):
            ...
            # auf den exakten Tag beschneiden
            sd = day
            ed = day + pd.Timedelta(days=1) - pd.Timedelta(milliseconds=1)
            full = df[sd:ed]
        else:
            # **Wichtig**: selbst bei leerem DataFrame muss der Index ein DatetimeIndex sein
