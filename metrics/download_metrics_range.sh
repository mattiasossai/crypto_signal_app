...
if [ $# -ne 5 ]; then
  echo "Usage: $0 METRIC START_DATE END_DATE PART_SYMBOL SYMBOL"
  exit 1
fi
...
while [ "$(date -d "$cur" +%s)" -lt "$end_ts" ]; do
  ...
  if [ -f "$FILE" ]; then
    ...
    case "$METRIC" in
      open_interest)
        # Richtiger Endpoint für historische Open Interest
        binance_url="https://fapi.binance.com/futures/data/openInterestHist?symbol=${SYMBOL}&period=1d&startTime=$(to_ms "$cur")&endTime=$(to_ms "$next")&limit=500"
        ;;
      funding_rate)
